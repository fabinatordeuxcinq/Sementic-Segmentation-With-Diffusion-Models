...
from .visualization_utils import make_displayable
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        :param model: the model, which takes a signal and a batch of timesteps
                      as input.
        :param x: the [B x C x ...] tensor at time t.
        ...
        B, C = x.shape[:2]
        assert t.shape == (
            B,), f"t must be a vector of size batch_size : {B.shape}"

        model_output = model(x, self._scale_timesteps(t), **model_kwargs)

        # loss is only calculated on the last channel, not on the input image
        x = x[:, -self.out_channels:, ...]

        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:

            model_var_values = model_output[:,
                                            self.in_channels:, ...]  # the img
            # added to replce before # the oise
            model_output = model_output[:, self.in_channels:, ...]
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, org, model_kwargs=None):
        ...
        """
        a, gradient = cond_fn(x, self._scale_timesteps(t),
                              org,  **model_kwargs)
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t,  model_kwargs=None):
        ...
        out["pred_xstart"] = self._predict_xstart_from_eps(
            x.detach(), t.detach(), eps)
        out["mean"], _, _ = self.q_posterior_mean_variance(
            x_start=out["pred_xstart"], x_t=x, t=t
        )
        return out, eps

    def sample_known(self, model, img, batch_size=1, fid=None):
        image_size = self.image_size
        channels = self.channels
        return self.p_sample_loop_known(model, (batch_size, channels, image_size, image_size), img, fid=fid)
    ...
    def p_sample(
        ...
        noise = th.rand_like(x[:, self.in_channels:, ...])
    ...
    def p_sample_loop_known(
        ...
        return final["sample"], None, img  # None was x_noisy
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            for i in indices:
                ...
                if i % 100 == 0:
                    print(f'sampling step : {i}/{len(indices)}', end='\r')
                    if self.tb_writer is not None :
                        display_img = make_displayable(img.squeeze().cpu()[self.in_channels:, ...])
                        self.tb_writer.add_image(f"Diffusion/image_{fid}_{i}", display_img, global_step=i, dataformats='HWC')
    ...
    def _vb_terms_bpd(
        ...
        x_start : start noise dim : [B, out_channels, ...]
        x_t : denoised noise dim : [B, out_channels, ...]
        :return: a dict with the following keys:
        ...
        assert decoder_nll.shape == x_start.shape, f"{decoder_nll.shape} {x_start.shape}"
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        :param x_start: the [N x (in_channels + out_channels) x ...] tensor of inputs.
        ...
        res = x_start[:, self.in_channels:, ...]
        if noise is None:
            # we want noise to be same shape as mask
            noise = th.randn_like(res)

        # add noise to the segmentation channel
        res_t = self.q_sample(res, t, noise=noise)

        x_t = x_start.float()

        # here 1 is the input dimension (1 for a single channel image)
        x_t[:, self.in_channels:, ...] = res_t.float()
        ...
        if self.loss_type in [LossType.MSE, LossType.RESCALED_MSE]:
            ...
            if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
                ...
                frozen_out = model_output.detach()
